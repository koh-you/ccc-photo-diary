...
AI 호출은 브라우저가 직접 OpenAI에 접속하지 않고, 로컬 Node 서버가 대신 호출합니다.
...
3. 작성 화면의 `OpenAI API 키` 칸에 `sk-...` 전체 키를 붙여넣습니다.
...
1. OpenAI API key 전체를 복사합니다. `sk-`로 시작해야 합니다.
...
서버 실행 전에 `OPENAI_API_KEY` 환경변수를 설정하세요.
...
모델은 기본값으로 `gpt-5.5`를 사용합니다. 바꾸고 싶으면 `OPENAI_MODEL`을 지정합니다.
...
서버 환경변수 설정이 번거로우면 작성 화면의 `OpenAI API 키` 칸에 키를 직접 입력해도 됩니다.

```text
앱 화면 API 키 입력
-> 로컬 Node 서버로 전달
-> 로컬 Node 서버가 OpenAI API 호출
```

이 키는 앱이 자동 저장하지 않습니다. 다만 같은 Wi-Fi의 다른 사람이 앱에 접근할 수 있는 환경에서는 API 키 입력 방식을 쓰지 않는 것이 좋습니다.

브라우저에서 OpenAI API를 완전히 직접 호출하는 방식은 CORS와 API 키 노출 문제 때문에 권장하지 않습니다. ChatGPT 웹 화면을 우회해서 제출하는 방식도 공식 API가 아니어서 안정적으로 만들기 어렵습니다.
...
- 앨범 안에서 과거 기록 보기
- 사진 촬영/선택 후 작성 화면 이동
- OpenAI Responses API 프록시 엔드포인트
- AI 초안 생성과 수동 반영
- 여러 사진 저장
- 여러 사진 대표 사진 선택
- 여러 사진 상세 갤러리 표시
